...
class AccountDatabaseBloc
    extends Bloc<AccountDatabaseEvent, AccountDatabaseState> with Getters {
  /// @nodoc
  AccountDatabaseBloc(
  ...
  Stream<AccountDatabaseState> mapEventToState(
    ...
    yield* event.map(
      accountLaunchedEVT: (event) async* {
        ...
        for (var i = 0; i < accountNavbarItemsKeys.length; i++) {
          layoutDataUpdated.add(<bool>[]);
          for (var j = 0; j < accountVerticalNavbarItemsKeys.length; j++) {
        ...
        if (callSuccess) {
          add(const AccountDatabaseEvent.indexChangedToSeriesPublishedEVT());
        }
      },
  ...
  /// @nodoc
  Stream<AccountDatabaseState> resetFailureOrSuccessOptions() async* {
    yield state.copyWith(
